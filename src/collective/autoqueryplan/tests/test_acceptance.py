...
from collective.autoqueryplan.config import HAVE_PLONE_5
from collective.autoqueryplan.testing import TASK_QUEUE_ZSERVER_FIXTURE
from collective.autoqueryplan.testing import REDIS_TASK_QUEUE_ZSERVER_FIXTURE
from collective.autoqueryplan.testing import ZSERVER_FIXTURE


class autoqueryplanFormLayer(PloneSandboxLayer):
    bases = (PLONE_FIXTURE,)

    def setUpZope(self, app, configurationContext):
        #import collective.autoqueryplan.pasplugin
        #xmlconfig.file('configure.zcml',
        #               collective.autoqueryplan.pasplugin,
        #               context=configurationContext)
        #z2.installProduct(app, 'collective.autoqueryplan.pasplugin')
        ...
        import collective.autoqueryplan.tests
        xmlconfig.file('test_acceptance.zcml',
                       collective.autoqueryplan.tests,
                       context=configurationContext)
    ...
    def setUpPloneSite(self, portal):
        ...
        #self.applyProfile(portal, 'collective.autoqueryplan.pasplugin:default')

TASK_QUEUE_FORM_FIXTURE = autoqueryplanFormLayer()
...
REDIS_TASK_QUEUE_ROBOT_TESTING = z2.FunctionalTesting(
    bases=(REDIS_TASK_QUEUE_ZSERVER_FIXTURE,
           ...
           TASK_QUEUE_FORM_FIXTURE,
           ZSERVER_FIXTURE),
    name='Redisautoqueryplan:Robot')


class IautoqueryplanForm(Interface):

    url = schema.ASCIILine(title=u"Path")


class autoqueryplanForm(form.Form):

    fields = field.Fields(IautoqueryplanForm)
    ...
    def handleQueue(self, action):
        ...
        if not _authenticator:
            autoqueryplan.add(data.get('url'))
        else:
            autoqueryplan.add(data.get('url'),
...
class IautoqueryplanEmailForm(Interface):

    message = schema.TextLine(title=u"Message")
    amount = schema.Int(title=u"Amount")


class autoqueryplanEmailForm(form.Form):

    fields = field.Fields(IautoqueryplanEmailForm)
    ...
    def handleQueue(self, action):
        ...
        for i in range(data['amount']):
            autoqueryplan.add('/{0:s}/send-email-view'.format(path), method='POST')
        plone_utils = getToolByName(self.context, 'plone_utils')
        plone_utils.addPortalMessage(
            "Queued {0:d} new email(s)".format(data['amount']))


class autoqueryplanEmailView(BrowserView):
